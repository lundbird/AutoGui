...
Func WriteScriptOff()
    ...
    _FileWriteToLine($sFilePath,1,"***Settings***" & @CRLF & "Library  SimpleGUI  " & @CRLF & @CRLF & "***Test cases***" & @CRLF & "Case 1",false)

	Local Const $pFilePath = @scriptdir & "\recordedActions.py"
	Local Const $dllPath = "../bin/GUILibrary.dll"
	_FileWriteToLine($pFilePath,1,"import clr" & @CRLF & "import os" & @CRLF & "clr.AddReference(os.path.abspath(" & '"' & $dllPath & '"' & "))" & @CRLF & "from GUILibrary import GUILibraryClass as sg" & @CRLF,false)
EndFunc
...
Func WriteRobotScript($obj,$objParent)
    ...
    if $LastWindow<>$ptitle Then
		...
		FileWrite($pFileOpen,"sg.setWindow(" & '"' & $ptitle & '"' & ")" & @CRLF)
		$LastWindow=$ptitle
	EndIf
